...
export default function OrgAgentsPage() {
  ...
  const fetchAgents = async () => {
    ...
    if (config.isMockMode) {
      setAgents([
        { id: "1", name: "Agent Smith", email: "agent@saziate.com", phone: "config.locality.code8039281234", createdAt: Date.now() - 360000000 },
        { id: "2", name: "Chinedu Okafor", email: "chinedu@saziate.com", phone: "config.locality.code8123456789", createdAt: Date.now() - 720000000 },
      ]);
      setLoading(false);
      return;
    }
  ...
  const executeDeactivate = async () => {
    ...
    try {
      const res = await fetch(`/api/v1/org/agents/${id}`, { method: "DELETE" });
      if (res.ok) {
        setAgents((prev) => prev.filter(a => a.id !== id));
      } else {
  ...
  return (
    <div>
        ...
        {loading ? (
          ...
          <div className="text-center py-12" style={{ padding: "3rem" }}>
            ...
            <p className="text-muted text-sm" style={{ maxWidth: "320px", margin: "0 auto 1.5rem" }}>
              Invite field agents to assign them service zones and start coordinating operations.
            </p>
            <button className="btn btn-primary" onClick={() => { setMessage(null); setShowModal(true); }}>
      ...
      <ConfirmModal
        ...
        title="Deactivate Agent"
        message={`Are you sure you want to deactivate ${confirmModal.name}? They will lose access to all your zones.`}
