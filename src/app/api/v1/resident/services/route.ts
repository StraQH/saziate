...
import { fieldLogs } from "@/db/schema";
...
export async function GET(req: Request) {
  ...
  try {
    ...
    if (config.isMockMode) {
      // Mock service logs for John Doe r1
      return new Response(
        JSON.stringify([
          {
            id: "col-001",
    ...
    // Query fieldLogs from D1
    const results = await db
      .select()
      .from(fieldLogs)
      .where(eq(fieldLogs.residentId, residentId))
      .orderBy(fieldLogs.loggedAt)
      .all();
