...
export function PayoutModal({ isOpen, onClose, onSuccess, showAlert }: PayoutModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (config.isMockMode) {
      ...
      onSuccess();
      showAlert("Mock Payout Initiated", `Payout of ${formatNaira(val)} simulated successfully.`);
      onClose();
      setLoading(false);
      return;
    }
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Request Payout">
      <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
        <p className="text-muted text-sm" style={{ marginBottom: "0.5rem" }}>
          Transfer funds from your available digital collections to your settlement bank account.
        </p>
