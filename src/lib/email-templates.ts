...
export const emailTemplates = {
    ...
    <div style="background-color: #F8FAFC; border: 1px solid #E2E8F0; border-radius: 8px; padding: 24px; margin: 24px 0;">
      <table width="100%" border="0" cellspacing="0" cellpadding="0" style="font-size: 14px; color: #334155;">
        ...
        <tr>
          ...
          <td align="right" style="padding: 8px 0; border-bottom: 1px solid #E2E8F0; font-weight: 700; color: #2563EB; font-size: 16px;">₦${totalAmount.toLocaleString(config.locality.locale)}</td>
        </tr>
        <tr>
  ...
  payoutConfirmation: (operatorName: string, amount: number, accountMask: string) => buildEmailWrapper("Payout Settled", `
    ...
    <p style="margin: 0 0 20px 0;">Your operator payout settlement of <strong style="color: #1E293B;">₦${amount.toLocaleString(config.locality.locale)}</strong> has been processed successfully to your settlement account ending in <strong>${accountMask}</strong>.</p>
  ...
  /**
   * Welcome Org Operator (Admin Onboarded)
   */
  welcomeOrgOperator: (name: string, tempPassword: string) => buildEmailWrapper("Welcome to Saziate", `
   ...
   */
  welcomeAgent: (name: string, orgName: string, tempPassword: string) => buildEmailWrapper("Welcome to Saziate", `
    <h2 style="color: #2563EB; font-size: 20px; font-weight: 700; margin: 0 0 16px 0;">Field Agent Profile Activated</h2>
    <p style="margin: 0 0 20px 0;">Welcome to Saziate, ${name}!</p>
    <p style="margin: 0 0 20px 0;">You have been successfully onboarded as a Field Agent representing <strong>${orgName}</strong>.</p>
    <p style="margin: 0 0 20px 0;">Please access the Field Agent mobile app application using your email and the temporary password below:</p>
    <div style="padding: 16px; background-color: #F8FAFC; border: 1px solid #E2E8F0; border-radius: 8px; font-size: 20px; font-weight: 700; text-align: center; letter-spacing: 2px; color: #1E293B; margin: 24px 0;">
      ${tempPassword}
    </div>
   ...
   * Invite Field Agent
   */
  inviteAgent: (orgName: string, inviteLink: string) => buildEmailWrapper("Join Saziate", `
    <h2 style="color: #2563EB; font-size: 20px; font-weight: 700; margin: 0 0 16px 0;">Invitation to Join ${orgName}</h2>
    <p style="margin: 0 0 20px 0;">You have been invited to join the field operations team at <strong>${orgName}</strong> as a Field Agent on the Saziate platform.</p>
    <p style="margin: 0 0 24px 0;">Please click the button below to accept the invitation and set up your agent credentials. Once registered, you will be able to log field services and verify payments on behalf of your operator.</p>
    <div align="center" style="margin: 24px 0;">
  ...
  invoiceReceipt: (residentName: string, amount: number, invoiceRef: string, transactionRef: string, serviceType: string = "waste_management") => buildEmailWrapper("Payment Receipt", `
    ...
    <p style="margin: 0; font-size: 14px; color: #64748B; font-style: italic;">Thank you for your prompt payment.</p>
  `),
